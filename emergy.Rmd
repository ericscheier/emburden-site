...
- Under-5 mortality: coef = +0.38 baseline (p = 1.5×10⁻³), +0.31
  with demog control (p = 2.2×10⁻³). A country importing 50% of
  merchandise as fuel loses ~0.19 extra U5 deaths per 1,000 live
  births per 10% oil-shock above its 3-year mean.
- Electricity access: coef = −0.18 baseline (p = 1.6×10⁻³),
  −0.15 with demog (p = 1.0×10⁻²). High-oil-import countries see
  access expansion slow during shocks — the shock cannibalises
...
donut-adjacent country snapshot from em_c_extended primary sources
(10 social-floor + 7 ecological-ceiling dimensions × 213 countries).
Quadrant snapshot:

| Quadrant | N |
|:---|---:|
| Safe & just (inside the donut) | 131 |
...
(+0.030, p=0.004), `sector_transport` (+0.028, p=0.005). Details at
[`docs/q3_35c_donut/`](https://github.com/ericscheier/emburdensynth/tree/global/docs/q3_35c_donut).

### Bundle Q3.36 — Finance alpha, full 21D donut, cross-benchmark validation
...
- `ppg_debt_usd ← sector_industrial`: ΔR²=+0.219, p=3×10⁻⁸ —
  industrial-sector energy share predicts PPG debt levels beyond
  GDP+region. Industrialisation is debt-financed at scale visible
  cross-sectionally.
- `debt_service_pct_gni ← heii_norm_p10`: ΔR²=+0.177, p=8×10⁻⁴ —
...
(ΔR²=+0.045, p=0.040) alongside `sector_industrial` (+0.017,
p=0.014) and `sector_transport` (+0.010, p=0.043). Social shortfall
stays baseline-saturated. Details at
...
across all 10 fuel commodities in Pink Sheet. **All four crude-
oil benchmarks show the same +0.27-0.29 coefficient on under-5
mortality with p<0.005**:
...
### Bundle Q3.37 — A proper Raworth-donut × SDG-wheel × emergy overlay

Until now, our "donut" visualisation was a horizontal-bar forest of
ΔR² values, projected onto donut framing but rendered as a coefficient
...
![Donut small multiples 12 countries](figures/donut_wheels/plot_country_donuts_smallmults.png)
...
![Donut trajectory grid 6 countries](figures/donut_wheels/plot_country_donut_trajectories_grid.png)
