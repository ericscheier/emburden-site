...
```{r fig-odum-overlay, fig.height=8.5, fig.width=14, fig.cap="HEII framework in Odum ESL overlaid on the Abel-2023 spectral hierarchy. Sources (sun/biomass/fossil/uranium circles) pinned at their raw primary-fuel transformity. Producers (bullet-nosed hexagons) pinned at their electricity-basis transformity (Bundle-A efficiency correction). Grid + Household symbols at the country-median grid transformity. Culture info in the Cultural Information lens; Culture configs at the top of the hierarchy. 213 countries plotted as regional dots at their grid transformity, sized by log(population)."}
...
# BASE spectral lenses (Abel fig 21)
lens_specs <- tibble::tribble(
  ~name,                       ~x0, ~a,   ~b,   ~fill,      ~lbl_x, ~lbl_y, ~lbl_size,
  "Atmosphere / Ecosystems",   2.0, 3.0,  5.5,  "#228B22",   0.5,   -4.0,   3.6,
  "Economic Production",       4.3, 3.6,  4.6,  "#F39C6B",   4.3,   -4.4,   3.4,
  "Primary Sector",            4.6, 1.3,  3.2,  "#8E44AD",   4.6,    3.9,   2.9,
  "Households Hierarchy",      5.8, 2.4,  3.4,  "#C0392B",   7.4,    4.1,   3.0,
  "Cultural Information",      8.0, 3.5,  2.8,  "#F39C12",   8.7,   -3.5,   3.0,
  "Culture Configurations",   12.5, 2.0,  1.6,  "#F1C40F",  12.5,   -2.2,   2.6)
make_lens <- function(x0, a, y0, b, n = 80, name = NA) {
  t <- seq(0, 2*pi, length.out = n)
  tibble::tibble(name = name, x = x0 + a * cos(t), y = y0 + b * sin(t))
}
lenses <- pmap_dfr(lens_specs |> select(name, x0, a, b, fill),
  function(name, x0, a, b, fill) cbind(make_lens(x0, a, 0, b, name = name), fill = fill))
region_pal <- c(
...
country_dots <- em_c |>
  ...
  mutate(x = log10(transformity_sej_per_J),
         ...
         y = -0.3 + (iso_num %% 7) * 0.20,
         reg = dplyr::coalesce(region_wb, "Unknown"),
         pop_size = pmax(log10(pmax(population, 1e5, na.rm = TRUE))-4, 0.1))
esl_sources <- tibble::tribble(~cx, ~cy, ~id, ~fill, ~label,
  log10(1),      -2.0, "src_sun",    "#F1C40F", "Sun/Wind/Rain",
  log10(25000),  -2.6, "src_bio",    "#228B22", "Biomass",
  log10(48000),  -3.1, "src_fossil", "#8A5A16", "Fossil",
  log10(200000), -3.1, "src_uran",   "#0F4D0F", "Uranium")
sym_sources <- pmap_dfr(esl_sources |> select(cx, cy, id, fill),
  function(cx, cy, id, fill) cbind(odum_circle(cx, cy, 0.22, id, n = 40), fill = fill))
esl_prod <- tibble::tribble(~cx, ~cy, ~id, ~fill, ~label,
  log10(1500),        2.4, "prod_wind",    "#228B22", "Wind",
  log10(3400),        1.9, "prod_pv",      "#F1C40F", "PV",
  log10(80000),       1.4, "prod_hydro",   "#1F77B4", "Hydro",
  log10(25000/0.28),  3.0, "prod_bio_e",   "#7B4E20", "Bio-e",
  log10(40000/0.35),  2.4, "prod_coal",    "#8A5A16", "Coal",
  log10(48000/0.50),  3.4, "prod_gas",     "#B5791B", "Gas",
  log10(54000/0.38),  3.9, "prod_oil",     "#C0392B", "Oil",
  log10(200000),      0.9, "prod_nuclear", "#0F4D0F", "Nuclear")
sym_prod <- pmap_dfr(esl_prod |> select(cx, cy, id, fill),
  function(cx, cy, id, fill) cbind(odum_producer(cx, cy, 0.7, 0.45, id), fill = fill))
esl_grid_x <- log10(med_tr) - 0.20
esl_hh_x   <- log10(med_tr) + 0.55
sym_grid   <- odum_storage(esl_grid_x, 0, 0.55, 0.55, "grid");    sym_grid$fill <- "#1F77B4"
sym_hh     <- odum_consumer(esl_hh_x, 0, 0.55, 0.55, "hh");        sym_hh$fill   <- "#C0392B"
sym_pgap <- odum_interaction(esl_hh_x + 0.85, 0.9, 0.35, "phys_gap"); sym_pgap$fill <- "#C0392B"
sym_egap <- odum_interaction(esl_hh_x + 0.85, -0.9, 0.35, "econ_gap"); sym_egap$fill <- "#F39C12"
sym_cult <- odum_consumer(8.0, 0, 0.8, 0.55, "cult"); sym_cult$fill <- "#8E44AD"
sym_config <- odum_consumer(12.5, 0, 0.75, 0.5, "config"); sym_config$fill <- "#F1C40F"
esl_all <- bind_rows(sym_sources, sym_prod, sym_grid, sym_hh,
                     sym_pgap, sym_egap, sym_cult, sym_config)
esl_labels <- bind_rows(
  esl_sources |> mutate(y = cy - 0.40) |>
    transmute(x = cx, y = y, label = label, size = 2.4),
  esl_prod |> mutate(y = cy + 0.45) |>
    transmute(x = cx, y = y, label = label, size = 2.4),
  tibble::tribble(~x, ~y, ~label, ~size,
    esl_grid_x, -0.55, "Grid",           2.5,
    esl_hh_x,   -0.60, "Household",      2.5,
    esl_hh_x + 0.85,  1.55, "Phys gap",  2.2,
    esl_hh_x + 0.85, -1.55, "Econ gap",  2.2,
    8.0,          -0.85, "Cultural info", 2.5,
    12.5,         -0.60, "Culture\ncfgs", 2.3))
ggplot() +
  ...
  geom_point(data = country_dots,
              aes(x = x, y = y, fill = reg, size = pop_size),
  ...
  new_scale_fill() +
  geom_polygon(data = esl_all, aes(x = x, y = y, group = id, fill = fill),
                alpha = 0.90, colour = BR[["ink"]], linewidth = 0.35) +
  scale_fill_identity() +
  geom_text(data = esl_labels, aes(x = x, y = y, label = label, size = size),
...
### Time-series — 60 years of transformity drift

Extended Ember (2000-2025, 213 countries) back to **1965 via EI
...
in emburdensynth). The 60-year animation shows country dots + median
Grid/HH position sliding as fuel-mix transforms:

![](figures/heii_overlay_extended_1965_2024.gif)

Data floor: 1965 is the honest global panel start (BP historical
series). Pre-1965 per-country per-fuel would require Etemad-Luciani
or Shift Project ingestion — flagged as a future improvement.
