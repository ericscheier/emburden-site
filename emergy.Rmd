...
per-country consumption from EIA International (Total energy consumption
by fuel product, 221 countries × 24 years, all in QBTU) and adds
World Bank / IEA sourced indicators (clean-cooking access, TFEC,
fossil / renewable share) — 212 of 213 emergy-pipeline countries now
carry a full fuel-mix vector. Sector consumption (transport /
industrial / residential / commercial / power) is *synthesized* from
observed fuel mix × published IEA fuel-to-sector heuristics; a proper
IEA World Energy Balances pull (paywalled — needs UNC library proxy)
would replace the synthesis with direct measurement.
...
```{r fig-odum-fuel-sector, fig.height=8, fig.width=13, fig.cap="Bundle Q3 — per-country fuel + sector consumption on the spectral hierarchy. LEFT half: for each country a stack of five fuel dots (petroleum / natural gas / coal / nuclear / renewables) at that fuel's canonical transformity, dot AREA proportional to that fuel's share of the country's primary energy consumption (EIA International 2000-2023 latest year). RIGHT half: per-country sector dots (transport / industrial / residential / commercial / power) synthesized from fuel mix × published IEA fuel-to-sector heuristics. Countries with `clean_cooking_access < 100%` also emit a large residential-biomass dot in the Households: P10 Subsistence tier. NON-GRID share (1 - sector_power) is annotated inline for the top-15 non-grid countries — the electricity-only pipeline misses 60-95% of their primary energy."}
...
sector_dots <- cx |>
  select(iso3, iso_num, reg,
         ...
         power = sector_power) |>
  pivot_longer(-c(iso3, iso_num, reg), names_to = "sector", values_to = "share") |>
...
ggplot() +
  ...
  annotate("text", x = 3.5, y = 4.55,
            label = "PER-COUNTRY SECTOR CONSUMPTION  (synthesized: fuel mix × IEA fuel-to-sector matrix)",
  ...
  new_scale_fill() +
  # Sector dots
  geom_point(data = sector_dots,
              aes(x = x, y = y, fill = fill, size = dot_area),
              shape = 22, colour = "white", stroke = 0.20, alpha = 0.80) +
  scale_fill_identity() +
...
- **EIA International** (api.eia.gov/v2/international, `activityId=2`
  ...
  4418 Renewables + other. 5,251 country-year rows.
- **World Bank WDI**: EG.CFT.ACCS.ZS (clean cooking access),
  EG.ELC.ACCS.ZS (electricity access), EG.FEC.RNEW.ZS (RE share of TFEC),
  EG.USE.PCAP.KG.OE (per-cap energy), EG.USE.COMM.FO.ZS (fossil share),
  EG.ELC.COAL/NGAS/PETR/FOSL.ZS (electricity fuel mix). 6,237 rows.
- **Sector shares** (transport / industrial / residential / commercial /
  power) are *synthesized* by multiplying observed fuel shares by
  the published IEA fuel-to-sector allocation matrix:
  petroleum → 60% transport / 15% industry / 10% residential / 5%
  commercial / 10% power; natural gas → 5 / 35 / 25 / 15 / 20;
  coal → 0 / 40 / 5 / 5 / 50; nuclear → 100% power; renewables → 5 /
  10 / 20 / 5 / 60. Direct measurement would need IEA World Energy
  Balances (paywalled — UNC library manual proxy is the follow-up
  path). Non-grid share = `1 − sector_power`.
